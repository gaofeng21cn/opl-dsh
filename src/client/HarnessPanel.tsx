...
const stateLabel:Record<string,string>={idle:'未开始',running:'执行中',waiting_approval:'等待授权',waiting_input:'等待回答',completed:'已完成',failed:'未完成',cancelled:'已取消',interrupted:'已中断'}
  ...
  return <div className={css.panel}>
    ...
    <div className={css.body}>
      <aside className={css.sidebar}><Button variant='outline' onClick={()=>{setSelected('');setSource(undefined)}}>新建组合对话</Button>
        {[...groups].map(([project,sessions])=><section key={project}><h3 title={project}>{project.split(/[\\/]/).filter(Boolean).at(-1)}</h3><p className={css.path}>{project}</p>{sessions.map(s=><button className={css.session} data-selected={selected===s.id} key={s.id} onClick={()=>{setSelected(s.id);setCwd(s.cwd);setCombination(s.combination)}}><strong>{s.title}</strong><span>{s.combination===GROK_COMBINATION?'Grok + Grok Build':'DeepSeek + DSH'} · {stateLabel[s.state]}</span></button>)}</section>)}
      </aside>
      <main className={css.conversation}>
        {session?<><div className={css.sessionHeader}><strong>{session.combination===GROK_COMBINATION?'Grok + Grok Build':'DeepSeek + DSH'}</strong><span>{stateLabel[session.state]}</span><Button variant='outline' disabled={busy||['running','waiting_approval','waiting_input'].includes(session.state)} onClick={()=>{setSource(session);setSelected('');setCwd(session.cwd);setCombination(session.combination===GROK_COMBINATION?DSH_COMBINATION:GROK_COMBINATION);setText('')}}>交给另一组合</Button><Button variant='outline' disabled={busy||!['running','waiting_approval','waiting_input'].includes(session.state)} onClick={()=>void act(()=>call('cancel',{sessionId:session.id}))}>取消任务</Button></div><p className={css.path}>{session.combination===GROK_COMBINATION?'模型 Grok 4.7 · 官方 Grok Build':'模型 DeepSeek-V4.1-Flash · 官方 DSH'} · {session.cwd} · 来源 {session.origin.kind} / {session.origin.sessionId}</p>
          ...
          {['waiting_approval','waiting_input'].includes(session.state)&&session.combination===DSH_COMBINATION&&<section className={css.approval} role='alert'><strong>DSH 子对话等待你的确认</strong><p>在左侧同项目中打开「DeepSeek · DSH · 组合协作」，通过官方授权或问题卡片处理后，结果会同步回这里。</p><p className={css.path}>{session.acpSessionId}</p>{close&&<Button variant='outline' onClick={close}>返回 DSH 对话</Button>}</section>}
