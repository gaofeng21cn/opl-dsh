...
await writeFile(join(output, 'package/lib/typert.host.js'), typert)
...
const peers = Object.fromEntries([...new Set(imports)].map(name => [name, name.startsWith('@deepseek-ai/dsh-') ? '0.1.7-rc.2' : '*']))
await writeFile(join(output, 'package/package.json'), JSON.stringify({
  name: '@one-person-lab/dsh-opl', version: '0.1.0', type: 'module', license: 'MIT',
...
execFileSync('tar', ['-czf', join(output, 'opl-dsh-enhancements-0.1.0.tgz'), '-C', output, 'package'])
const archive = join(output, 'opl-dsh-enhancements-0.1.0.tgz')
const digest = createHash('sha256').update(await readFile(archive)).digest('hex')
const name = 'opl-dsh-enhancements-0.1.0-'+digest.slice(0,12)+'.tgz'
...
const payloadFiles = {}
for (const file of ['install.command', 'install.mjs', 'profile.cjs', 'setup.mjs', 'skill/SKILL.md', 'skill/control.mjs', name]) {
...
const artifact = { name, sha256: digest, suiteSha256, payloadFiles }
