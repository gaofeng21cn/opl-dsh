...
test('tarball checksum and contents bind the installer to an external plugin',async()=>{
 ...
 assert.equal(pkg.name,'@one-person-lab/dsh-opl');assert.equal(pkg.peerDependencies['@deepseek-ai/dsh-llm'],'*')
 assert.equal(manifest.version,manifest.officialVersion);assert.equal(manifest.enhancementVersion,pkg.version)
